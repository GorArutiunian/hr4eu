  ...
  return (
    <div className="flex h-full w-full min-h-[340px] items-center justify-center bg-gradient-to-br from-slate-800 to-slate-900 p-6">
      ...
      >
        ...
        <text x={xClassic + barWidth / 2} y={padding.top + chartH + 24} textAnchor="middle" fill="#94a3b8" fontSize="14" fontWeight="700">Classic employment</text>
        <text x={xClassic + barWidth / 2} y={padding.top + chartH + 44} textAnchor="middle" fill="#64748b" fontSize="12">High cost</text>
        ...
        <text x={xWithUs + barWidth / 2} y={padding.top + chartH + 24} textAnchor="middle" fill="#e2e8f0" fontSize="14" fontWeight="700">With us</text>
        <text x={xWithUs + barWidth / 2} y={padding.top + chartH + 44} textAnchor="middle" fill="#93c5fd" fontSize="12" fontWeight="700">Lower cost ✓</text>

        {/* "Better" badge */}
        <rect x={xWithUs - 4} y={withUsY - 52} width={barWidth + 8} height="22" rx="6" fill="#22c55e" />
        <text x={xWithUs + barWidth / 2} y={withUsY - 37} textAnchor="middle" fill="white" fontSize="12" fontWeight="800">BETTER CHOICE</text>

        {/* Bottom sentence: yearly savings */}
        <text x={w / 2} y={h - 24} textAnchor="middle" fill="#4ade80" fontSize="16" fontWeight="700">
          You will save {savingsPerYear.toLocaleString("cs-CZ")} CZK per year for one employee
        </text>
      </svg>
    </div>
  );
